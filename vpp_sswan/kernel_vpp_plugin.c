...
struct private_kernel_vpp_ipsec;

typedef struct kernel_vpp_listener {
	listener_t public;
	struct private_kernel_vpp_ipsec *ipsec;
} kernel_vpp_listener_t;

typedef struct private_kernel_vpp_ipsec {
	...
	kernel_ipsec_sa_id_t *id, kernel_ipsec_add_sa_t *data)
{
  ...
  VAC_LOG("api: ipsec_sad_entry_add spi=%.8x", htonl(id->spi));
  ...
  if (err.num && err.type == NAAS_ERR_VNET)
	...
	DBG1(DBG_KNL, "put SA_CHILD %#H == %#H with SPI %.8x by its sa_id %x",
			id->src, id->dst, htonl(id->spi), sad_id);
	...
	kernel_ipsec_sa_id_t *id, kernel_ipsec_del_sa_t *data)
{
  ...
  VAC_LOG("api: ipsec_sad_entry_del");
  err = NAAS_API_INVOKE (mp, rmp);
  naas_api_msg_free (rmp);
  if (err.num && err.type == NAAS_ERR_ERRNO)
    {
		...
		kernel_ipsec_sa_id_t *id, kernel_ipsec_update_sa_t *data)
{
	...
	DBG1(DBG_KNL, "update SA_CHILD %#H == %#H with SPI %.8x to %#H == %#H",
	...
	rv = _del_sa(this, id, data);
	if (rv == SUCCESS) {
		rv = _add_sa(this, id, data);
	}
...
METHOD(kernel_ipsec_t, ipsec_destroy, void, private_kernel_vpp_ipsec_t *this)
{
...
static void
tunnel_protect_update(kernel_vpp_listener_t *this, uint32_t unique_id,
		uint32_t i_spi, sa_t *i_sa, uint32_t o_spi, sa_t *o_sa)
{
	uint32_t sw_if_index, i_sa_id, o_sa_id;
	naas_err_t err;

	if (i_sa == NULL && o_sa == NULL) {
		DBG1(DBG_IKE, "CHILD_SA with SPIs %.8x_i %.8x_o not found", i_spi, o_spi);
		return;
	}
	...
	if (sw_if_index == ~0) {
		...
		naas_api_sw_interface_set_unnumbered(1, this->ipsec->loop_sw_if_index,	sw_if_index);
	}

	if (i_sa != NULL && o_sa != NULL) {
		naas_api_ipsec_tunnel_protect_update(sw_if_index, i_sa->sa_id, o_sa->sa_id);
		return;
	}

	err = naas_api_ipsec_tunnel_protect_dump(sw_if_index, &i_sa_id, &o_sa_id);
	if (err.num) {
		return;
	}
	if (i_sa != NULL) {
	...
	naas_api_ipsec_tunnel_protect_update(sw_if_index, i_sa_id, o_sa_id);
}
...
{
	sa_t *i_sa, *o_sa;
	uint32_t sw_if_index, i_spi, o_spi, unique_id;
	protocol_id_t proto;
	kernel_ipsec_sa_id_t key;

	unique_id = ike_sa->get_unique_id(ike_sa);

	VAC_LOG("child_state_change unique_id=%u, state=%u", unique_id, state);

	switch (state) {
	case CHILD_INSTALLED:
		proto = child_sa->get_protocol(child_sa);

		key.src = ike_sa->get_my_host(ike_sa);
		key.dst = ike_sa->get_other_host(ike_sa);
		key.proto = proto == PROTO_ESP ? IPPROTO_ESP : IPPROTO_AH;
		o_spi = child_sa->get_spi(child_sa, FALSE);
		key.spi = o_spi;
		o_sa = this->ipsec->sas->get(this->ipsec->sas, &key);

		key.src = ike_sa->get_other_host(ike_sa);
		key.dst = ike_sa->get_my_host(ike_sa);
		i_spi = child_sa->get_spi(child_sa, TRUE);
		key.spi = i_spi;
		i_sa = this->ipsec->sas->get(this->ipsec->sas, &key);

		tunnel_protect_update(this, unique_id, i_spi, i_sa, o_spi, o_sa);
...
kernel_vpp_listener_t *
kernel_vpp_listener_create(private_kernel_vpp_ipsec_t *ipsec)
{
